...
function shiftStatusText() {
  ...
  if (onDuty === "myluv") {
    return `Shift sekarang (${label}): giliran ayangku ngeshift 💖.`;
  }
  if (onDuty === "someone") {
    return `Shift sekarang (${label}): bukan ayangku yang shift.`;
  }
...
const romanticReplies = [
  "Aku juga suwayang kamu poll 💕",
  "Cintaku padamu takkan pernah padam 🥰",
  "I love you moree sayangggg. Always 💞",
  "Kamu satu-satunya notifikasi yang selalu aku tunggu 💘",
  "Sayang kamu sampe sundul langit 💖",
  "Setiap ayangku shift, aku selalu doain kamu semoga lancar balesin DM nya 🌷",
  "Mwach muah mwawchhh mmmwwaaahhh 💋💋💋💋",
  "Aku cinta kamu selamanyaaaaa ❤️",
  "Kamu + Aku = InsyaAllah penuh masjid",
  "Kamu seperti Al Waqiah ayah 23 💕",
];

bot.command("iloveu", async (ctx) => {
  await logEvent("command", "/iloveu", ctx.chat.id);
  const reply =
